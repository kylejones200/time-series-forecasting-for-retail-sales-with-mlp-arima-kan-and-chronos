...
def add_forecast_label(y_vals, label, offset=0):
    ax.text(end_date + pd.DateOffset(months=1), y_vals[-1] + offset, label, fontsize=9, va='center', ha='left')

def evaluate(y, yhat):
    return (mean_squared_error(y, yhat) / 1000000.0, mean_absolute_error(y, yhat) / 1000000.0, mape(y, yhat))

def main_step_002(X_test_t, X_train_t, arima_time, loss_fn, mean_val, mlp, opt, pred_arima, prediction_length, series, split, start_mlp, std_val, window, y_test, y_train_t) -> None:
    for _ in range(200):
        mlp.train()
        opt.zero_grad()
    ...
    prepare_x_train_lstm(X_test_t, X_train_t, arima_time, kan_time, loss_fn, mean_val, mlp_time, pred_arima, prediction_length, series, split, std_val, y_test, y_train_t)
    main_step_003(forecast_index, pred_arima, pred_chronos, pred_kan, pred_lstm, pred_mlp, prediction_start_date, results, series_zoom, valid_length)

def main_step_003(forecast_index, pred_arima, pred_chronos, pred_kan, pred_lstm, pred_mlp, prediction_start_date, results, series_zoom, valid_length) -> None:
...
def prepare_x_train_lstm(X_test_t, X_train_t, arima_time, kan_time, loss_fn, mean_val, mlp_time, pred_arima, prediction_length, series, split, std_val, y_test, y_train_t) -> None:
    ...
    chronos_pipe = ChronosPipeline.from_pretrained('amazon/chronos-t5-large', device_map=device, torch_dtype=torch.bfloat16 if torch.cuda.is_available() else torch.float32)
    ...
    results = pd.DataFrame([evaluate(y_actual, p) + (t,) for p, t in zip(preds, times)], columns=['RMSE (M USD)', 'MAE (M USD)', 'MAPE (%)', 'Train Time (s)'], index=models)
    ...
    valid_length = min(prediction_length, len(pred_arima), len(pred_mlp), len(pred_kan), len(pred_lstm), len(pred_chronos))
...
def prepare_split() -> None:
    ...
    loss_fn = nn.MSELoss()
    start_mlp = time.time()
    main_step_002(X_test_t, X_train_t, arima_time, loss_fn, mean_val, mlp, opt, pred_arima, prediction_length, series, split, start_mlp, std_val, window, y_test, y_train_t)
